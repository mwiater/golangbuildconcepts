#!/bin/bash
#
# From root of project, run: `bash scripts/golang_build_linux64-subscription.sh`
...
echo -e "${CYANBOLD}Building native Linux Go binary:${RESET} ${GREENBOLD}go build -ldflags "-X main.AppReleaseVersion=\$APPRELEASEVERSION" -tags "subscription" -o bin/golangbuildconcepts-subscription .${RESET}"
