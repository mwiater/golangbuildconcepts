...
export APPRELEASEVERSION=$(git rev-list -1 HEAD) && go build -ldflags "-X main.AppReleaseVersion=$APPRELEASEVERSION" -o bin/golangbuildconcepts .
status=$?
if test $status -ne 0
then
...
echo -e "${CYANBOLD}Running Go app:${RESET} ${GREENBOLD}./bin/golangbuildconcepts${RESET}"
./bin/golangbuildconcepts